...
class Trainer:
    def __init__(
        ...
        # self.device = torch.device('cpu') # for debugging
...
class ContextModelTrainer(Trainer) : 
    def __init__(
        self,
        model,
        train_dataloader,
        ...
        super(ContextModelTrainer, self).__init__(
            model,
            train_dataloader,
            eval_dataloader,
            test_dataloader,
    ...
    def iteration(self, epoch, dataloader, mode="train"):
        ...
        if mode == "train":
            ...
            for i, batch in rec_data_iter:
                # 0. batch_data will be sent into the device(GPU or CPU)
                batch = tuple(t.to(self.device) for t in batch) # batch_size , 6(n_columns)
                # user_idx, item_idx, year_idx, director_idx, genre_idx, writer_idx = batch[0]
                input_pos = batch[0]
                target_pos = batch[1]
                # _, input_ids, target_pos, target_neg, _ = batch
                # Binary cross_entropy
                output = self.model(input_pos)
                loss_fn = torch.nn.BCELoss()
                loss = loss_fn(output, target_pos)
                # loss = self.cross_entropy(output, target_pos)
                self.optim.zero_grad()
                loss.backward()
                self.optim.step()
            ...
            for i, batch in rec_data_iter:
                ...
                loss_fn = torch.nn.BCELoss()
                valid_loss += loss_fn(predictions, target).item()
                
                answer_list.append(target.cpu())
                # pred_list.append(predictions)
                preds = torch.tensor(np.where(predictions.cpu()>0.5, 1, 0))
            ...
            avg_valid_loss = valid_loss/len(rec_data_iter)
            ...
            if (patience > 10) : 
                torch.save(self.model.state_dict(), '/opt/ml/bk/code/output/model_bk.pt')
            elif (patience <= 10) & (epoch==self.args.epochs) : 
                torch.save(self.model.state_dict(), '/opt/ml/bk/code/output/model_bk.pt')
