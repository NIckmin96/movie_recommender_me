...
class FeatureLinear(nn.Module) : 
    def __init__(self, args, output_dim=1) : 
        ...
        self.fc = torch.nn.Embedding(sum(self.field_dims), self.output_dim)
        self.bias = torch.nn.Parameter(torch.zeros((self.output_dim,)))
        self.offsets = np.array((0, *np.cumsum(self.field_dims)[:-1]), dtype = np.int32) # encoding이 0부터 시작하기 때문에 +1
        
    def forward(self,x) : 
        x = x + x.new_tensor(torch.tensor(self.offsets)).to(self.args.device).unsqueeze(0) # 1,num_features
        # x = x + x.new_tensor(self.offsets).unsqueeze(0) # 1,num_features
...
class FeaturesEmbedding(nn.Module) : 
    def __init__(self, args) : 
        ...
        self.embedding = torch.nn.Embedding(sum(self.field_dims), self.emb_dim)
        self.offsets = np.array((0, *np.cumsum(self.field_dims)[:-1]), dtype = np.int32) # encoding이 0부터 시작하기 때문에 +1
        torch.nn.init.xavier_uniform_(self.embedding.weight.data)
        
    def forward(self, x) : 
        x = x + x.new_tensor(torch.tensor(self.offsets)).to(self.args.device).unsqueeze(0)
        # x = x + x.new_tensor(self.offsets).unsqueeze(0)
        return self.embedding(x)
    
class FactorizationMachine(nn.Module) : 
    def __init__(self, args, reduce_sum = True) : 
        super().__init__()
...
class FM(nn.Module) : 
    def __init__(self, args) : 
        super().__init__()
        self.args = args
        self.linear = FeatureLinear(args)
        self.embedding = FeaturesEmbedding(args)
        self.fm = FactorizationMachine(args)
