...
def recall_at_k(actual, predicted, topk):
    sum_recall = 0.0
    num_users = len(predicted) # num_batch
    true_users = 0
    for i in range(num_users):
        act_set = set(actual[i])
        pred_set = set(predicted[i][:topk])
...
def recall_at_k_fm(actual, predicted, topk):
    sum_recall = 0.0
    num_batch = len(predicted) # num_batch
    tp_fn = 0
    tp = 0
    for i in range(num_batch):
        tp_fn += len(predicted[i][predicted[i]==1])
        tp += actual[i][predicted[i]==1].sum()
